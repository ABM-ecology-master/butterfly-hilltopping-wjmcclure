...
to setup
  ...
  [
    set used? false
    ;Use sines to create 4 hills
    set elevation 200 + (100 * (sin (pxcor / max-pxcor * 360 * 1.5 ) + sin (pycor / max-pycor * 360 * 1.5 )))
    ;Add some noise to elevation
    ;set elevation elevation + random-float 4
    ...
    if elevation-labels [
      set plabel precision elevation 0
...
to go
  ...
  if ticks >= 1000 [
    ...
    export-plot "Corridor Width" (word "butterfly_hilltopping_data/width_over_time_q_" q ".csv")
    stop
  ]
end
