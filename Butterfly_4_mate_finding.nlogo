...
to go
  ...
  if ticks >= 1000 [
    ;Get a common date-and-time to index results
    let index date-and-time

    ;Report the final corridor width in a text box
    ;Save corridor width time series data to a csv file.
    ;The particulars of NLogo require you to have a directory named "butterfly_hilltopping_data" already created.
