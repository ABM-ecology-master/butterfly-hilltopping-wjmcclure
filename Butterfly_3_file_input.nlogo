globals [
;q ;probability of hilltopping behavior
]
patches-own [
  elevation
  used?
...
to setup
  ca
  ;Read in elevation data from a txt file.
  ; The text file is already processed for netlogo. X ranges from 0 to 201 and Y ranges from 0 to 153.
  file-open "processed_elevation.txt"
  ...
  [
    ...
    ;Color patches by elevation. Only label them if asked.
  ...
  [
    set size 2
    set color red
    ; Start butterflies absolutely randomly
    setxy random-pxcor random-pycor

    ; Start butterflies in a corner
    ;setxy max-pxcor max-pycor

    ; Start butterflies randomly in the middle x range (when x ranges from 0 to 150)
    ;setxy ((random 6) - 3 + 75) random-pycor

    ; Record starting patch
...
to go
  ...
  plot corridor-width
  tick
  ;End simuation after 1000 ticks.
  ;Report the final corridor width in a text box
  ;Save corridor width time series data to a csv file. The particulars of NLogo require you to have a directory named "butterfly_hilltopping_data" already created.
  if ticks >= 1000 [
    output-type "Corridor width: " output-type corridor-width
    export-plot "Corridor Width" (word "butterfly_hilltopping_data/input_elevation/width_over_time_q_" q ".csv")
    stop
  ]
end
...
to move ; turtle procedure
  ...
  [uphill elevation] ; native method, meaning "move to neighboring patch with highest elevation." Aka hilltop.
  [move-to one-of neighbors]
  ; Tell patch it's been used, then let it turn red.
  ask patch-here [
    set used? true
    ;set pcolor red
  ]
...
to-report corridor-width
  let num-patches-visited count patches with [used? = true]
